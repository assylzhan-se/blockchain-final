...
contract Verify is Script {
    ...
    function _verifyTimelock(address timelockAddr, address governorAddr) internal {
        ...
        _check(
            "Timelock.minDelay == 2 days", delay == EXPECTED_TIMELOCK_DELAY, string.concat("got ", vm.toString(delay))
        );
    ...
    function _verifyGovernor(address govAddr, address tokenAddr, address timelockAddr) internal {
        ...
        _check("Governor.votingDelay == 7200", gov.votingDelay() == EXPECTED_VOTING_DELAY, "");

        _check("Governor.votingPeriod == 50400", gov.votingPeriod() == EXPECTED_VOTING_PERIOD, "");

        _check("Governor.quorumNumerator == 4", gov.quorumNumerator() == EXPECTED_QUORUM_NUMERATOR, "");

        _check("Governor.token == govToken", address(gov.token()) == tokenAddr, "");

        _check("Governor.timelock == timelock", address(gov.timelock()) == timelockAddr, "");
    }
    ...
    function _verifyFeeVault(address vaultAddr, address ammAddr) internal {
        ...
        _check("AMM has FEE_DEPOSITOR_ROLE on FeeVault", fv.hasRole(fv.FEE_DEPOSITOR_ROLE(), ammAddr), "");
    }
    ...
    function _verifyAMM(address ammAddr, address marketAddr) internal {
        ...
        _check("PredictionMarket has MARKET_MANAGER_ROLE on AMM", a.hasRole(a.MARKET_MANAGER_ROLE(), marketAddr), "");
    }
